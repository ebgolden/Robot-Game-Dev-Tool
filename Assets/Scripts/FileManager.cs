...
public class FileManager
{
    ...
    public void writeToFile(CreatedPartsData createdPartsData, string data, string path)
    {
        this.path = path + "/";
        writeToFile(createdPartsData, data);
    }

    private void writeToFile(CreatedPartsData createdPartsData, string data)
    {
        if (createdPartsData != default)
        {
            string fileName = "createdparts";
            List<string> stringList = new List<string>();
            foreach (string id in createdPartsData.partIds)
                stringList.Add(id);
            string encryptionString = getEncryptionString(stringList);
            char encryptionCharacter = getEncryptionCharacter(encryptionString);
            int encryptionValue = getEncryptionValue(encryptionCharacter);
    ...
    public void writeToFile(TypeData typeData, string data, string path)
    {
        this.path = path + "/";
        writeToFile(typeData, data);
    }

    private void writeToFile(TypeData typeData, string data)
    {
        if (typeData != default)
        {
            string fileName = "typedata";
            List<string> stringList = new List<string>();
            stringList.Add("p");
            string encryptionString = getEncryptionString(stringList);
            char encryptionCharacter = getEncryptionCharacter(encryptionString);
